...
- **本番(Supabase Cloud)で `email rate limit exceeded` / 登録後にアプリへ遷移しない**: `db push` は **Auth 設定を同期しない**。ローカル `config.toml` は `enable_confirmations=false` でも、**Cloud は既定でメール確認 ON** → 確認メール送信でレート上限＋即セッション無しで `/` へ遷移しない（本アプリは確認メール導線なし）。→ ダッシュボード **Authentication → Providers → Email → 「Confirm email」OFF**。原則は [[managed-config-outside-migrations]]、手順は [deploy.md](deploy.md) §6。
